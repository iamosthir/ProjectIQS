...
async function load() {
    ...
    try {
        ...
        const { data } = await fanGroupsApi.verifications(params);
        rows.value = data.data;
    } catch (e) { toastErr(e); } finally { loading.value = false; }
}
async function approve(r) {
    try { await fanGroupsApi.approveVerification(r.id); toast.add({ severity: 'success', summary: t('clubs.approved'), life: 2000 }); await load(); }
    catch (e) { toastErr(e); }
}
function openReject(r) { current.value = r; rejectNote.value = ''; rejectDialog.value = true; }
async function reject() {
    try { await fanGroupsApi.rejectVerification(current.value.id, rejectNote.value); rejectDialog.value = false; await load(); }
    catch (e) { toastErr(e); }
}
onMounted(load);
</script>
...
<template>
    <div>
        <PageHeader :title="t('fanGroups.verifications')" :subtitle="t('fanGroups.verificationsSubtitle')" icon="pi pi-verified" />
        ...
        <div class="rounded-2xl border border-surface-200 bg-surface-0 dark:border-surface-800 dark:bg-surface-900">
            ...
            <DataTable :value="rows" :loading="loading" data-key="id" class="text-sm">
                ...
                <Column :header="t('fanGroups.title')">
                    <template #body="{ data }">
                        <p class="font-medium">{{ data.fan_group?.name }}</p>
                        <p class="text-xs text-surface-500">{{ data.user?.name || data.user?.phone }} · {{ data.method }}</p>
                    </template>
                </Column>
